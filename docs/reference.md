...
The Chinese tech theme also changes the floor plan: desks in a row form one long connected desk, every agent's bunk stands right behind its chair, and the master sits in a walled office with a large dark red desk and chair, a money tree by the entrance, red banners hung across the room (drawn at random from more than five hundred company slogans, a fresh set every time the office view is opened), a countdown board on the left wall, street-style CCTV poles watching the floor and the toilets, and a stopwatch over each stall that runs while it is occupied. Speech bubbles turn into green WeChat-style bubbles, every random line is in Chinese (more than five hundred fighting-spirit lines while the team is paused, more than five hundred "give me work" lines while a usage limit holds, plus Chinese nothing-to-do, hello, goodbye and tantrum lines), once a minute, when nobody is talking, the master delivers one of more than five hundred motivational lines for ten seconds, and half of the agents who find nothing to do take a smoke break by the entrance instead of going to bed: half of them squat there, the rest drift between spots near the door, and all of them rant about management, the pay and the company (more than five hundred lines) on the way out and between drags, for a couple of minutes before turning in.

An API error knocks an agent out where it stands in every theme. Two medics in white coats and surgical caps then come in through the door with a case, kneel by the casualty and work on it where it lies (chest compressions from one, the open case and a defibrillator shock every few seconds from the other, with the usual shouting) until the agent's next cycle brings it round; then they pick up the case and leave. When several agents are down, the same pair treats them one after another in the order they went down. In the Chinese office half of them despair instead: they say one of more than five hundred bleak lines every twenty seconds or so and either lie face down on their desk or stand banging their head against the nearest wall until the error clears. Each stall there also has a timer: after fifteen seconds of use a red alarm light flashes over it until the occupant leaves.
...
| DeepSeek and Ollama | | | | Both are Anthropic-compatible providers driven by the API backend's loop. **DeepSeek** (`deepseek`): set `DEEPSEEK_API_KEY`; requests go to `https://api.deepseek.com/anthropic`; models `deepseek-v4-pro` and `deepseek-flash` (1M context) priced at peak-hour list rates. **Ollama** (`ollama`): a local server at `OLLAMA_HOST` (default `http://127.0.0.1:11434`, Ollama 0.14 or newer); models are discovered from the server, or set `HUNTUN_OLLAMA_MODELS="qwen3:27b@65536,llama4"` with the context each should be assumed to have (default `HUNTUN_OLLAMA_CONTEXT`, 32768; start `ollama serve` with a matching `OLLAMA_CONTEXT_LENGTH`). Compatible providers get no Anthropic-only extras (effort, adaptive thinking, server-side fallbacks, web search/fetch server tools, eager input streaming); set `HUNTUN_COMPAT_THINKING=on` to request extended thinking with a `HUNTUN_COMPAT_THINKING_BUDGET` (8192) budget. Any field a provider rejects with a 400 is dropped and remembered. Structured answers (goal check, team plan) accept JSON in text when the model does not call the tool. Reliability depends on the model's tool-calling quality: try one agent before scaling up. |
| vLLM | | | | **vLLM** (`vllm`): a server speaking the OpenAI Chat Completions API at `VLLM_BASE_URL` (default `http://127.0.0.1:8000/v1`; `/v1` is added when missing), with `VLLM_API_KEY` sent as a Bearer token when set. Its own tool-use loop over `/chat/completions` drives Huntun's file, search, shell and team tools (no web search or fetch); the message history is saved after every step like the API backend's. Start vLLM with `--enable-auto-tool-choice --tool-call-parser <parser for the model>`, otherwise every cycle fails with a message saying so; the goal check and team plan use a named tool choice (guided decoding) and accept JSON in text as a fallback. Models and their context (`max_model_len`) are discovered from `/v1/models`, or set `HUNTUN_VLLM_MODELS="Qwen/Qwen3-32B@40960,other"` (default context `HUNTUN_VLLM_CONTEXT`, 32768). Output is capped at a quarter of the context (at most `HUNTUN_MAX_TOKENS`); a request the window cannot fit is retried without the cap, and one that overflows even so restarts the cycle's conversation from the task. Effort is not sent; reasoning (`reasoning_content`, or `<think>` tags in the reply) shows as thinking in the Live panel. `HUNTUN_VLLM_TIMEOUT` (1800 s) bounds one call. No per-token cost. SGLang, LM Studio and llama.cpp's `llama-server` work the same way. |
...
| Models the master picks from | Opus 5.5, Opus 5, Sonnet 5, Haiku 4.5 | `gpt-5.3-codex` (extend with `HUNTUN_CODEX_MODELS=a,b`) | Opus 5.5, Opus 5, Sonnet 5, Haiku 4.5 |
...
huntun/backends/vllm.py       vLLM backend (OpenAI Chat Completions tool loop with on-disk transcript)
