...
**Office view.** The button above the Team panel switches the centre column to a Game Boy style office in the classic four-shade green: a top-down tile map with desks and PCs, a meeting table, bunks, plants and doors, drawn at native 16-pixel tiles and scaled by whole pixels. Every agent is an overworld-style sprite (hair and hats vary per agent; the master wears a crown) that walks tile by tile, four directions, through the doorways. Working agents sit at their desks facing the PC, idle or paused agents sleep in the bunks, and an agent that just posted on the board gets an exclamation mark, walks to the meeting table, and its post scrolls into the dialog box at the bottom, typewriter style. Click a sprite to open its details. The header shows only the project name; hover it for the goal.
