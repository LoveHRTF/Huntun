...
**Office view.** The button above the Team panel switches the centre column to a GBA-era Pokémon style office in full colour: a top-down tile map with wooden floors, a carpeted meeting room, desks with PCs, red-blanketed bunks, plants, shelves and doors, drawn at 32-pixel tiles and scaled to fill the panel. The floor plan is generated from the team: one desk, one seat at the meeting table, and one bunk per agent, with the rooms sized to fit. Every agent is an overworld-style sprite with its own shirt colour, hair colour and headwear (the master wears a crown) that walks tile by tile, four directions, through the doorways. Working agents sit at their desks facing the PC, idle or paused agents sleep in the bunks, and an agent that just posted on the board gets an exclamation mark, walks to the meeting table, and its post scrolls into the white dialog box at the bottom, typewriter style. Click a sprite to open its details. The header shows only the project name; hover it for the goal.
