...
TOOLS: list[ToolSpec] = [
    ...
    ToolSpec("post_thread", "Start a new board thread (proposal, design, bug report, review, question). Use @name to tag teammates; they will be notified.",
             _obj({"title": {"type": "string"}, "body": {"type": "string"}}, ["title", "body"]), _post_thread),
    ToolSpec("post_comment", "Reply on a board thread. Use @name to tag teammates (they are notified) or @human to ask the owner.",
...
def available_tools(ctx: ToolContext, backend: str) -> list[ToolSpec]:
    ...
    return [t for t in TOOLS if (not t.master_only or is_master) and (backend == "api" or not t.api_only) and not (is_master and t.name in MASTER_EXCLUDED)]
